...
router.get("/incidents", async (req, res): Promise<void> => {
  const rows = await db.select().from(incidentsTable).where(eq(incidentsTable.companyId, tenantCompanyId(req))).orderBy(incidentsTable.createdAt);
...
router.post("/incidents", async (req, res): Promise<void> => {
  ...
  const [incident] = await db.insert(incidentsTable).values({ ...parsed.data, companyId: tenantCompanyId(req), statut: "ouvert" }).returning();
...
router.patch("/incidents/:id", async (req, res): Promise<void> => {
  ...
  if (parsed.data.statut === "resolu" && !parsed.data.dateResolution) updateData.dateResolution = new Date();
  const [incident] = await db.update(incidentsTable).set(updateData)
    .where(and(eq(incidentsTable.id, id), eq(incidentsTable.companyId, tenantCompanyId(req)))).returning();
...
router.delete("/incidents/:id", async (req, res): Promise<void> => {
  const id = parseInt(req.params.id);
  const [incident] = await db.delete(incidentsTable)
    .where(and(eq(incidentsTable.id, id), eq(incidentsTable.companyId, tenantCompanyId(req)))).returning();
  if (!incident) { res.status(404).json({ error: "Incident not found" }); return; }
  res.sendStatus(204);
});
