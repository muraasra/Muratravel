...
router.get("/finance/transactions", async (req, res): Promise<void> => {
  ...
  const rows = companyId
    ? await db.select().from(transactionsTable).where(eq(transactionsTable.companyId, companyId)).orderBy(transactionsTable.createdAt)
...
router.get("/finance/summary", async (req, res): Promise<void> => {
  const companyId = req.query.companyId ? parseInt(req.query.companyId as string) : undefined;
  const cond = companyId ? eq(transactionsTable.companyId, companyId) : sql`1=1`;
